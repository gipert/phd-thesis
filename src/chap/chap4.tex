...
10, jumping from $\sim$2 to $\sim$~20 (\fillme{cross-check with betta}). This large
...
\newpar
Not all the posterior distributions for the aforementioned pdfs report non-zero estimates
of background counts in the spectra. Notably, posteriors for \kvz\ above the array and on
the \bege\ \nplus\ contact are peaked at zero and are thus removed from the fit. The
exclusion of \kvz\ on the \bege\ \nplus\ contact, which has not been reported in the fit
before analysis cuts (\cref{chap:bkg:raw:ph2}), is attributed to the novel transition
layer model, which significantly affects the pdf shape above the FEP. The \b\ spectrum
component above 1525~keV, indeed, is now higher and shows less compatibility with
experimental data. The \Co\ component is also compatible with zero, possibly because of
the high LAr veto efficiency on a background contamination which was already low before
the cut (c.f.r.~\cref{tab:bkg:raw:ph2:gmodel:results}). Nevertheless, the component is not
removed from the fit because of its clear non-zero mode.
\newpar
The results of the analysis are shown in \cref{fig:bkg:lar:ph2:results} and
\cref{tab:bkg:lar:ph2:results} (\fillme{table if I have time}). The background
decomposition in \cref{fig:bkg:lar:ph2:results} is presented together with the residuals
plot and the 68\%, 95\% and 99\% Poissonian probability intervals of the best fit model.
Smallest intervals are now used instead of central intervals
(c.f.r.~\cref{chap:bkg:raw:ph2})~\cite{Aggarwal2011}, and the visually-appealing
normalization in units of $\sigma$ (as in e.g.~\cref{fig:bkg:raw:ph2:gmodel:results}) is
not possible anymore. Already from a visual inspection of the residuals, the goodness of
fit is not satisfactory. In particular, discrepancies between the best-fit model and the
data are visible in \enrGeII\ right below the potassium peaks and in \enrBEGeII\ around
the potassium Compton shoulder region. These systematic discrepancies can be
well-attributed to uncertainties in the Monte Carlo LAr veto model. \fillme{is this true?
update to final fit results}
...
\begin{figure}
  ...
  \caption{%
    ...
    smallest 60\%, 95\% and \%99 confidence intervals. \fillme{some blocks look odd,
    check. logx?}
