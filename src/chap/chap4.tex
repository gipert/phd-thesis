...
equal to 68 and has been performed with a \Th\ source in July \yr{2016}, while the second one
...
\begin{table}
  ...
  \caption{%
    ...
    runs~68 (July \yr{2016}, \Th\ source) and 76 (February \yr{2017}, \Ra\ source). The random
...
locations (e.g.~\Ac\ in holders, cables, etc.) is expected. The following set of pdfs
has been therefore selected: \nnbb\ in germanium, \kvn\ close (mini-shrouds)
...
\newpar
\fillme{numbers} The background indices extracted from data after the LAr veto cut are
...
\begin{figure}
  ...
  \begin{tikzpicture}[anchor=south west]
    ...
    \node at (0, 0) {\includegraphics{plots/bkg/raw/ph2/results/gmodel/BI-bar.pdf}};

    \node at (0,-1) {\scshape After the LAr veto cut};
    \draw (0,-1) -- (\textwidth,-1);
  ...
  \caption{%
    ...
    The background decompositions are performed with a reduced set of pdfs, to make a
