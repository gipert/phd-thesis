...
publicly available on GitHub\footnote{%
  The \texttt{gerda-fitter} software: \url{https://github.com/gipert/gerda-fitter}, see
  \cref{footnote:gerda-fitter}.
}. Since the global event suppression factor built in the pdfs after the LAr veto cut
(i.e.~the pdf normalization) cannot be fully trusted, no priors from screening measurement
data have been set. \fillme{explain this better maybe somewhere else?} A uniform prior is
assigned to all fit parameters (including the \nnbb\ fit parameter, which is proportional
to $1/\thalftwoM$).
...
fit. The \Co\ component is also compatible with zero, possibly because of the high LAr
veto efficiency on a background contamination which was already low before the cut
(c.f.r.~\cref{tab:bkg:raw:ph2:gmodel:results}). Nevertheless, the component is not removed
from the fit because of its clear non-zero mode.
\newpar
