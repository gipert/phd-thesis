...
\cref{sec:bkg:lar:ph2:gmodel}, the signal-to-background ratio improves by a factor of 10
...
results are presented and discussed in the last \cref{sec:2nbb-ana:results}.
...
\begin{table}
  ...
  \caption{%
    ...
    extracted from toy Monte Carlo data sets. \fillme{numbers here}
...
\begin{description}
  ...
  \item[Theoretical \nnbb\ decay model] The theoretical calculations of the \nnbb\
    ...
    in~\cite{Ponkratenko2000} and references therein. In similar works from the
    past~\cite{Agostini2015a, Agostini2013c}in fact the systematic uncertainty contribution
    ...
    $<1\%$ effect, however there is no guarantee that the impact on the current analysis
    is at the same, second-order, level. The lower background and higher statistics with
    respect to the \phaseone\ data set require indeed a second evaluation. Calculations
    in the Primakoff-Rosen approximation are known to yield results which are less
    accurate than other techniques developed in recent years (i.e.~those implemented in
    \decayzero). Results from recent theoretical advances~\cite{Kotila2012} are more
    suitable to this comparison. \fillme{SSD vs HSD? Go ahead\ldots}
  ...
  \item[Other sources] The energy scale and resolution is another potential source of
    ...
    e.g.~\cref{fig:gerda:calib-desc}), this contribution is negligible.
    \newpar
    \fillme{Do we use the \onbb\ number for the \nnbb\ LAr survival probability? There's a
    small uncertainty there.}
...
C.L.~lower limits for new-physics processes. \fillme{eannamoooooooooooo}
