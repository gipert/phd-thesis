...
\begin{table}
  ...
  \input{tab/2nbb-ana/sensitivity-onbbx}
\end{table}
...
systematically to the final analysis uncertainty (both for \nnbb\ half-life and
new-physics limits) must be considered. Until now, the test statistics studies have only
considered the effect of Poisson fluctuations in the bin contents, as the generative
...
\begin{description}[wide]
  ...
  \item[\gesix\ active exposure] The detector active volume and enrichment fraction has
    ...
    estimate. Since the other systematic uncertainty sources contribute at the
    sub-percent level, the \gesix\ active exposure estimation results to be the major
    source of uncertainty in the standard \nnbb\ half-life determination. In contrast, it
    is a second-order contribution to the systematic uncertainty on new-physics limits,
    as the latter is typically of about 10\% or higher.
...
\nuc{Xe}{139} experiments~\cite{Gando2012, Albert2014a}\footnote{%
...
the \ga\ coupling constant\footnote{%
  ...
  these are very old and rough calculations (published in \yr{1995}) that do not take into
  account the last \yr{25} years of nuclear theory developments. We are in contact with
...
\begin{itemize}
  ...
  \item The extracted $\thalftwoM = (\fillme{?}) \cdot 10^{21}$~yr is one of the most precise
    measurements of a double-beta decay half-life ever reported, together with recent
    ...
    are placed in the $10^{22}$--$10^{23}$~yr range, improving the \phaseone\ results by a
