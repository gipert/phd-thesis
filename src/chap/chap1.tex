...
suggested their existence in \yr{1930}~\cite{Brown1978}. Despite their weakly interacting
nature, which let them elude direct detection until \yr{1965}~\cite{Cowan1956},
...
theories, as a matter of fact, predict that the asymmetry is eventually produced by a
...
and \cref{sec:nbb:2nbbLV} other exotic double-beta decay modes that contemplate the
emission of additional particles (i.e.~the Majorons) or break the Lorentz symmetry are
considered. Finally, aspects concerning the detection of double-beta decay events in real
experimental settings will be examined in \cref{sec:nbb:exp}.
...
left, in which there is an accessible $2^+$ excited state of \nuc{Se}{76}. However, due to
...
Neutrinoless double-beta decay processes (\onbb) of the types\footnote{%
...
equal importance of matter and anti-matter in the Standard Model.  In \yr{1967},
...
As mentioned in \cref{sec:nbb:0nbb}, the Majorana nature of the neutrino leads to the
...
decay~\cite{Carone1993}, which we will also refer to as Majoron in the latter. A decay
...
Since the Standard Model is known to provide a successful description
...
constructed from the latter which allows for arbitrary coordinate-independent Lorentz
...
oscillations because they are produced by ``oscillation-free'' operators that modify all
...
effects that could be enormous compared to those suppressed by the $m_W/m_P$ ratio and that
...
coefficients are likely to dominate at accessible energies and can be sensitively measured
...
all orientations, which implies that only isotropic effects are observable and hence
...
\begin{equation}\label{eq:nbb:totwidth}
  ...
  \end{split}
\end{equation}
The total decay rate can be therefore expressed as a sum of two separate rates through a
perturbation:
\[
...
amplitude) and the decay rate relative to the perturbation $\delta\Gamma$ is determined by
the \aof\ coefficient:
\[
...
\begin{table}
  ...
  \caption{%
    ...
    coefficient from double-beta decay experiments.
...
unnecessary, as demonstrated in the case of \nuc{Te}{130}~\cite{Alduino2017}.

\blocktitle{backgrounds}
To achieve the ambitious goal of operating in a background-free regime,
...
several type of backgrounds. Prompt muon interactions typically deposit a large amount of
energy in detectors and can be easily vetoed. Backgrounds from secondary neutrons and
cosmogenically-activated isotopes are the main worries. The latter can be avoided by
...
Neutrinoless double-beta decay has a characteristic event topology, in which two $\sim$MeV
...
\begin{description}[wide]
  ...
  \item[Bolometers] Bolometers are cryogenic calorimeters that operate at temperatures of
    ...
    from many materials that include double-beta decay isotopes, e.g.~TeO\textinf{2},
    \nuc{Cd}{116}WO\textinf{4}, Zn\nuc{Se}{82}, \nuc{Ca}{40}\nuc{Mo}{100}O\textinf{4},
    Zn\nuc{Mo}{100}O\textinf{4} and  Li\textinf{2}\nuc{Mo}{100}O\textinf{4}.  Advantages
    ...
    abundance of \nuc{Te}{130}, CUPID~\cite{Wang2015}, which explores the possibility to
    improve the background rejection in CUORE through active particle discrimination, and
    \textsc{AMoRE}, a \nuc{Mo}{100}-based experiment~\cite{Kim2015}. The ultimate
  ...
  \item[Scintillators] The main appeal of organic scintillators for \onbb\ searches is
    ...
    starting the new \kamlandzen-800 phase, that will load 750~kg of enriched xenon. In
    ...
    Searching \onbb\ decay with \nuc{Te}{130} is the main goal of another liquid
  ...
  \item[Tracking Calorimeters] The \textsc{SuperNEMO}~\cite{Arnold2010} unique
    ...
    The projected sensitivity with \nuc{Se}{82} for a full \textsc{SuperNEMO} is $1.2
    \cdot 10^{26}$~yr.

\end{description}

A comparison of the discovery sensitivity of current and planned neutrinoless double-beta
...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{plots/theory/next-gen-discovery-sens.pdf}
  \caption{%
    ...
    \nuc{Mo}{100} (green), \nuc{Te}{130} (blue), \nuc{Xe}{136} (yellow) and \gesix\ (red)
