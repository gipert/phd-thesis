...
\[
  ...
  \end{array}
\]
Since $Q_\text{EC} = Q_{\upbeta^+}+2m_e$, the electron-capture process (EC) can
occur even if the $\upbeta^+$ process is energetically forbidden ($Q_{\upbeta^+} <
...
\partitle{nuclear \\ matrix \\ elements}
...
\end{inparaenum}
The unpolarized transition probability
then reads:
...
inferred~\cite{Caldwell2017, Agostini2017, Ge2017}. An optimistic picture can
...
while nuclear term \nmez\ estimations are in the 1--10 range but remain
terribly affected by large error bars. The situation for the latter is depicted
in \cref{fig:nbb:nme}. As already mentioned in \cref{sec:nbb:2nbb}, the
quenching problem affects also \onbb. Recent studies suggest that there might
be less quenching needed (not more than 20--30\%) in processes with large
momentum transfer such as \onbb, but there is not yet consensus in the
literature. A reduced $g_A$ implies a longer \thalfzero, which is undesirable
...
\begin{figure}
  ...
  \caption{%
    A representative compilation of nuclear matrix element calculations
    with an unquenched $g_A=1.27$ for different isotopes.
  }\label{fig:nbb:nme}
\end{figure}
...
\[
  G_\alpha^{0\upnu\upchi(\upchi)}(Q_{\upbeta\upbeta}, Z) \sim {(Q_{\upbeta\upbeta} - K)}^n \;.
\]
...
\partitle{experimental \\ limits}
\fillme{to be updated} Conservative constraints on the \aof\ coefficient have
...
spectrum~\cite{Diaz:2013saa}. An outside analysis of data from the two
experiments gives $|\aofooM| < 2 \cdot 10^{-8}$~GeV for both of them.
Constraints on the values of \aof\ have also been extracted from studies of
threshold effects in pion and kaon decays~\cite{SMEneutrinos}, yielding $\aofM
< 1.9 \cdot 10^{-7}$~GeV as the best upper limit.
...
\nnbb\ spectrum is used to obtain a limit at the 90\% C.L, which is $\aofM <
7.6 \cdot 10^{-6}$~GeV. This is the first search for this parameter that fully
accounts for experimental backgrounds and detector-related systematic
uncertainties.
...
\partitle{experimental \\ approaches}
...
\begin{description}[wide]
  \item[Semiconductors] One of the most promising approaches for scaling to
    ...
    LEGEND experimental program~\cite{Abgrall2017}.
  ...
  \item[Bolometers] Bolometers are cryogenic calorimeters that operate at
    ...
    abundance of $^{130}$Te, CUPID~\cite{Wang2015}, which explores the
  ...
  \item[Time Projection Chambers] The time projection chamber (TPC) is an
    ...
    nEXO~\cite{Kharusi2018}. Planned high-pressure xenon gas-phase TPC projects
    are NEXT~\cite{Lopez-March2017} and PandaX-III~\cite{Chen2016}. Two-phase
    liquid-xenon detectors, popular for dark matter searches, such as
  ...
  \item[Scintillators] The main appeal of organic scintillators for \onbb
    ...
    resolution in the KamLAND2-Zen experiment. Also inorganic CaF$_2$
    scintillators have been always receiving interest, because of the high
    $^{48}$Ca \qbb\ of 4.27~MeV. However, finding a cost effective process to
    enrich the isotope, whose natural abundance is only $\sim$0.2\%, is still a
    challenge. The CANDLES series~\cite{Umehara2015} of \onbb\ searches at the
    Kamioka Observatory is actively exploring this experimental approach.
  ...
  \item[Tracking Calorimeters] The SuperNEMO~\cite{Arnold2010} unique
    ...
    difficult to scale up to large exposures.
\end{description}
