...
processes, but also through the processes \fillme{write more}
\[
  \begin{array}{lrl}
    \text{EC}\beta^+: &
...
rule for simple \b-decay. To a good approximation, the decay width can be
...
element (NME) $\mathcal{M}^{2\nu}$ deals with the nuclear structure of the
transition and is notoriously much more difficult to evaluate.

\marginnote{phase \\ space}
...
two anti-neutrinos. Denoting the 4-momentum of the two electrons and the two
anti-neutrinos by $p^\alpha_i = (E_i,\mathbf{p}_i)$ and $q^\alpha_i =
(\omega_i,\mathbf{q}_i)$, respectively ($i=1,2$), the differential decay
amplitude reads:
\[
  \begin{split}
    \text{d}\Gamma =
...
\marginnote{nuclear \\ matrix \\ element}
...
$g_A$ that is necessary to reproduce the observable quantities of nuclear
decays, particularly \b\ and \nnbb\ decays.
