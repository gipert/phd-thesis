...
using variable-width blocks, and is therefore attractive when representing data that span
many different orders of magnitude. An exhaustive comparison between this and similar
...
point using only the information from the previous ones. In this way, the algorithm
...
\end{lstlisting}
What the algorithm does when adding a given data point \m{k} is to add to the optimal
\fillme{fillme}

\blocktitle{\gerda\ \\ data}
Here's an application to \gerda\ data.
...
\end{center}
The Julia code is reported also in the following.
