...
history of the achievements of \gesix\ experiments is presented in
\cref{img:exp:ge76-history}. The evolution of half-life lower limits and background index
is reported in the top panel, \mbb\ ranges and \gesix\ exposure at the bottom.  Different
markers on the exposure data series are used to indicate the type of experiment (natural
or enriched germanium, above or under ground). The technology advance and the half-life
lower limit achievements over more than \yr{50}~years is impressive. In
\cref{fig:exp:gerda-history} the evolution of the sensitivity and the limits set by
\gerda\ as a function of the accumulated exposure is shown. The linear sensitivity gain
demonstrates the background-free conditions in which \gerda\ has been operating for most
of its run time. The \gerda\ data taking officially ended in November \yr{2019} after hitting
the target total background-free exposure of 100~\kgyr\ and establishing itself as the
leading experiment in the field in terms of lowest background level ever achieved around
\qbb~\cite{Kermaidic2020, Agostini2021}.
...
Since the start of the data taking in \yr{2008}, the experiment, located in hall~A of the
...
germanium in the form of five inverted-coaxial geometry detectors was
...
\gerdafinallimit. In the following, when generally referring to the \phasetwo\ period,
...
\begin{figure}
  ...
  \caption{%
    ...
    \thalfzero\ 90\% C.L.~lower limits set by \gerda\ as a function of the accumulated
...
collaboration, born in October \yr{2016} a fusion of \gerda\ and
\majorana\footnote{%
...
The \gerda\ \phasetwo\ detector array is organized in 7 vertical strings, holding 40 detectors in
...
\phasetwop\ in \yr{2018} four enriched inverted-coaxial \IC{} detectors were introduced to
...
The \gerda\ \phasetwo\ detectors before the \yr{2018} upgrade can be classified according to
...
\begin{figure}
  ...
  \hspace{0.5cm}%
  \includegraphics[width=0.65\textwidth]{gedet/phII-array-2D.pdf}
  \vspace{0.5cm}

  \includegraphics[height=7cm]{gedet/phIIp-array.png}%
  \hspace{0.5cm}%
  \includegraphics[width=0.65\textwidth]{gedet/phIIp-array-2D.pdf}
  \caption[placeholder]{%
    The \gerda\ \phasetwo\ detector array. On the top: setup from the start of Phase II
    (December \yr{2015}). On the bottom: \phasetwop\ setup after the \yr{2018} upgrade works. The
    ...
    created through the \m{gedet-plots} library\footnotemark{}.
...
\phasetwo.
\newpar
The LAr veto system was upgraded in \yr{2018} for \phasetwop\ to achieve a higher vetoing
...
\begin{figure}
  \centering
  \includegraphics[width=0.35\textwidth]{gedet/phII-array-calib.png}
  \caption{%
...
A FADC system (SIS 3301 Struck) records traces from germanium detectors (40), PMTs (16)
...
\begin{figure}
  ...
  \caption{%
    ...
    purity in \yr{2018} might be attributed to maintenance works of the cryogenic
    infrastructure.
...
detectors, independently on the status of the energy calibration\footnote{%
  ...
  leakage current, etc.) or `anti-coincidence'. The latter is set when the detector is
...
developed for \bege\ and inverted-coaxial (\icoax) from one side, and semi-coaxial
(\scoax) from the other, to be applied after the LAr veto cut. For the first class a
simple univariate cut was sufficient, while for the latter two techniques were worked out,
one based on neural networks and one on the analysis of the rise time of the pulses.  In
order to avoid systematic effects calibration, training and evaluation of the PSD methods
should be performed on pulses with energies close to these of the expected signal at \qbb.
In practice, appropriate event sets are extracted from the weekly \Th\ calibration
spectra.  The PSD methods applied to the \gerda\ data are briefly outlined in the
following. The interested reader is referred to~\cite{Agostini2021a} for a detailed
...
figure of merit defined as the product of the \nnbb\ signal survival square-probability
and the \a-events rejection probability. The \nnbb\ and \a-event test samples are obtained
from physics data by selecting data in the $[1.0, 1.3]$~MeV and $>3.5$~MeV energy regions,
respectively, after \annmse\ and LAr veto cuts.  The \onbb-signal efficiency of the RT cut
is assumed to be the same as for the \nnbb\ decays and hence estimated to 82.3\%, with an
uncertainty of the order of 1\%~\cite{Lazzaro2019}.
...
\section{\texorpdfstring{Final \gerda\ results on the search for \onbb\ decay}{Final \gerda\ results on the search for 0nbb decay}}%
\label{sec:gerda:ana}
...
suitable analytical function, a function interpolation is performed to obtain the energy
...
respect to its value right after a calibration. Time periods in which a detector
...
of most recent nuclear matrix elements calculations, a limit of $m_{\upbeta\upbeta} = 79 -
...
\begin{itemize}
  \item The \gerda\ experiment is searching for neutrinoless double-beta decay with
    ...
    and passive shield to background events. The data taking stopped in \yr{2019} after
    collecting more than 100~\kgyr\ of data and surpassing the \powtenyr{26} signal
  ...
  \item Several background reduction techniques have been worked out to achieve a
    background index on the order of \powctsper{-3} in the \onbb\ region of interest. A
  ...
  \item The full \gerda\ data set is analyzed in frequentist and Bayesian frameworks. Data
    ...
    \measurement{5.2}{1.6}{1.3}~\powctsper{-4} achieved in the region of interest. \gerda\
