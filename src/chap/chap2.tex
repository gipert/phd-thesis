...
Since the start of data taking in \yr{2008}, the experiment, located in hall~A of the
...
collaboration, born in October \yr{2016} as a fusion of \gerda\ and
\majorana\footnote{%
...
The \gerdatwo\ detector array is organized in 7 vertical strings, holding 40 detectors in
...
The \gerda\ \phasetwo\ detectors before the \yr{2018} upgrade can be classified according
...
\begin{figure}
  ...
  \caption{%
    Top view of the \phasetwo\ array, with the three calibration sources \m{C1}, \m{C2}
    and \m{C3}. Drawings have been created through the \m{gedet-plots}
    library\textsuperscript{\ref{footnote:gedetplots}}.
...
\sloppy which is still sufficient to generate a non-negligible background of the order of
...
trigger the scintillation process. These background types mainly include \g-rays from \Ra\
...
In semi-coaxial detectors the length of the holes drift path depends on the location of
...
effective data set energy resolution as a function of energy is then determined by fitting
...
\powtenyr{26}. At the time of writing, \gerda\ is providing the best sensitivity and the
