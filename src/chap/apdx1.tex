...
negligible, compared to \Arl~\cite{?}. The \Kr\ contribution has also been discarded
because it is highly localized at 514~keV, in correspondence with its only \g\ line, and
does not contribute significantly to the rest of the energy spectrum. To find the
...
\cref{fig:gedetav:ar39-results}. Note that data is not available for some detectors that
have not been deployed in \phasetwop\ (\ANG{1}) or were switched off because of hardware
instabilities (\ANG{5} and \IC{48B}).

\blocktitle{results}
Starting from the FCCD values, it is interesting to note the great compatibility between
...
However, no convincing hypothesis have been formulated nor tested yet.
