...
\cref{fig:bkg:raw:ph2:pdfs:gmodel}, \cref{fig:bkg:raw:ph2:pdfs:amodel},
\cref{fig:bkg:raw:ph2:pdfs:kmodel:K40} and \cref{fig:bkg:raw:ph2:pdfs:kmodel:K42} and will
...
\begin{figure}
  \centering
  \subfloat[%
  \Pa\ and \Ac\ contaminations in the close vicinity of the detectors
  (mini-shrouds).\label{fig:bkg:raw:ph2:pdfs:gmodel:other}%
  ...
  \subfloat[%
    ...
    \gesix\ \nnbb\ decay.\label{fig:bkg:raw:ph2:pdfs:gmodel:other2}%
  ...
  \subfloat[%
    ...
    (mini-shrouds) the detector.\label{fig:bkg:raw:ph2:pdfs:gmodel:U}%
    ]{\includegraphics[width=0.48\textwidth]{plots/bkg/raw/ph2/pdfs/gmodel-pdfs-U.pdf}}

  \subfloat[%
    \kvn\ contamination close to the detector array (on the mini-shrouds), at a higher
    radial distance (on the fiber shroud) and higher vertical distance (on the copper
  ...
  \caption{%
    The pdfs in the full energy domain. The pdfs for the \enrGeII\ ($\enrBEGeII +
    \enrCoaxII$) (in fully opaque colors) and the \enrGeII\ (in shaded colors) data sets
    relative to different background sources. For visualization purposes a variable
    binning is adopted. \fillme{mention bayesian blocks} All pdfs are normalized to the
...
\cref{apdx:assay} and~\cite{Agostini2018a}, Sec.~5. All possible background sources taken
into consideration in this analysis are described in detail below.
...
100~cts/detector. In \cref{fig:bkg:raw:ph2:pdfs:kmodel:K40} the expected counts per detector
...
energy spectra (for details see \cref{sec:bkg:raw:ph2:kmodel}).
...
\cref{fig:bkg:raw:ph2:pdfs:gmodel:K42} the pdfs of \kvn\ and \kvz\ in energy are prone to
...
location of the two potassium isotopes. In total the \Mone\ data contains 4472~cts in
$1461 \pm 4$~keV and 6718~cts in $1525 \pm 4$~keV while the \Mtwo\ events contain 554~cts in
$1461 \pm 6$~keV and 865~cts in $1525 \pm 6$~keV, respectively. An analysis of the number of
events in the two potassium \g\ lines in each detector (and detector pair) is used to
