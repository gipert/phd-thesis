...
A precise knowledge of background intensity and distribution is essential to searches for
faint signals. One main assumption of the \onbb-decay signal analysis is the distribution
of background events in the analysis window around \qbb\ being uniform, except for known
\Tl\ and \Bil\ \g\ lines. The primary role of the background model is to verify this
assumption by exploiting data outside this energy region or filtered with a different
event selection. The background model is also complementary to assay measurements in
determining the location of the most dangerous background sources and learn how to improve
experimental design and material selection in similar future projects. Lastly, a good
background model allows to statistically isolate \nnbb-decay events, estimate the
half-life of the process and study their distribution as a source of potential new-physics
effects (see \cref{chap:theory}).
\newpar
...
the first part of \phasetwo, namely, for data acquired starting from December \yr{2015} to
March \yr{2018}. Single-detector (or multiplicity-one, abbreviated \Mone) events and
...
The probability density functions (\pdf{}s) used to model contributions to the energy spectra
...
(\cref{fig:bkg:raw:ph2:pdfs:gmodel:other2}). The latter decays with a half-life of
...
as of September \yr{2014}~\cite{Agostini2015e}. Extrapolating the expected impurities to the
...
likelihood function adopted for the present analysis combines the $N_\text{dat}=3$ data
sets \enrBEGeII, \enrCoaxII\ and \enrGeII:
...
the \Po\ peak is modeled with a mixture of \pdf{}s obtained from simulations with different
...
\begin{figure}
  ...
  \caption{%
    \a-model \pdf{}s in the full energy domain corresponding to different \pplus\ contact
    thicknesses. Contaminants are simulated on the outer contact surface. The red \pdf\
    corresponds to the absence of dead layer. All \pdf{}s are normalized to the number of
    simulated decays.
...
is sufficient. We determine a best-fit value of 100~nm and 500~nm, respectively,
consistent with the manifacturer technical specifications. The estimated \pvalue\ for
\enrBEGeII\ is 0.2 whereas the \pvalue\ for \enrCoaxII\ is 0.3. The dominant spectral
component below 4.5~MeV is due to degraded \a\ events which extends down to the ROI.
...
to be best described by a mixture of \pdf{}s obtained assuming different \pplus\ contact
...
% The values for each background contribution are given in
% \cref{tab:bkg:raw:ph2:gmodel:results} and are displayed as a fraction of the total
...
reported in \cref{tab:bkg:raw:ph2:gmodel:results} should be taken \emph{cum grano salis}.
\newpar
...
of \phasetwo, for a total exposure of 60.2~\kgyr. In April \yr{2018} the data taking was
...
germanium\footnote{%
  The \Zn\ present in \icoax\ crystals is due to the \nuc{Ge}{70}(n, \a{}2n)\Zn\ reaction,
  induced by cosmic rays~\cite{Cebrian2010, Horiguchi1983}. It disintegrates mainly by
  electron capture to the 1115~keV excited level of \nuc{Cu}{65} with a half-life of
  244~days. \icoax\ detectors have been deployed in \gerda\ short after being produced, so
  the 1124~keV line (\g\ de-excitation of \nuc{Cu}{65} coincident with K-shell X-rays) can
...
\begin{figure}
  \centering
  \includegraphics{plots/bkg/raw/ph2p/results/gmodel/results-roi-fixbw.pdf}
  \caption{%
    The background decomposition of the last \gexpophasetwopbkg\ of \gerdatwo\ before
...
\begin{figure}
  ...
  \caption{%
    Marginalized posterior distributions of the \nnbb-decay half-lives reconstructed from
    the three \enrBEGeIIp, \enrSCoaxIIp\ and \enrICoaxIIp\ data sets separately. The \pdf{}s
...
for each detector type, the \a\ model \pdf{}s and \Zn\ in \icoax\ detectors.
\newpar
