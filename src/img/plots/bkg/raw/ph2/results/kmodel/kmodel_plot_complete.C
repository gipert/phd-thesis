...
#include "kmodel_common_complete.hpp"
...
void plot_histos(TH1D* data_raw,
    ...
    for (auto it = comp_sum_raw.begin(); it != comp_sum_raw.end(); ) {
        if ((*it)->GetEntries() == 0) {
            comp_sum_raw.erase(it);
        } else it++;
    }
...
//    dataPad->SetGridx();
    ...
    // GERDA watermark
    auto gerdawtr = new TLatex(0.992, 0.85, "GERDA 19-07");
    gerdawtr->SetNDC();
    ...
    canvas->Update();
    if (ds >= 5 and ds <= 9 ) canvas->SaveAs(Form("kmodel-2d-ds%d-complete.pdf", ds));
    else                      canvas->SaveAs(Form("kmodel-1d-ds%d-complete.pdf", ds));
...
void plot_1d(int dataset = 0,
    ...
    // get histograms
    int ds = dataset;
    auto f = new TFile("kmodel-results-complete.root");
...
void plot_asymmetries() {
    ...
    // fetch results tree
    TFile * f = new TFile("kmodel-results-complete.root");
    TTree * t = (TTree*) f->Get("resultsTree");

    char name[50];
    double marg_mode;

    t->SetBranchAddress("name", &name[0]);
    t->SetBranchAddress("marg_mode", &marg_mode);

    int n = t->GetEntries();

    // results map
    std::map<string,double> map_res;

    for(int e = 0; e < n; e++) {
        t->GetEntry(e);
        std::string sname = name;
        map_res[sname] = marg_mode;
    }

    // create two maps to hold the results
    std::map<std::string,double> map_k40;
    std::map<std::string,double> map_k42;

    // make graphs
    std::vector<double> x = { 0,   6,  6,0,  -6,  -6,0};
    std::vector<double> y = {-7,-3.5,3.5,7, 3.5,-3.5,0};
    std::vector<double> z_k40, z_k42;
    for(auto ac : acnames_k40) z_k40.push_back( map_res[ac] );
    for(auto ac : acnames_k42) z_k42.push_back( map_res[ac] );
    z_k40.at(6) += z_k40.back();
    z_k40.pop_back();

    //TGraph2D * gr_k40 = new TGraph2D(7, &x[0], &y[0], &z_k40[0]);
    //TGraph2D * gr_k42 = new TGraph2D(7, &x[0], &y[0], &z_k42[0]);

    // plot'em
    std::ofstream ofile_k40_k42("kmodel-asym.txt");

    for(int e = 0; e < 7; e++) {
        ofile_k40_k42 << x[e] << "\t" << y[e] << "\t" << z_k40[e] << "\t" << z_k42[e] << "\n";
    }

    ofile_k40_k42.close();

    // TCanvas * c = new TCanvas("asym","asym",800,800);
    // gr_k40->Draw("P0");
    // gr_k42->Draw("P0same");
    // c->SaveAs("kmodel-asym.pdf");

    return;
}

void kmodel_plot_complete() {
